...
func main() {
	configPath := flag.String("config", "configs/auth.json", "auth config file path")
	flag.Parse()

	cfg, err := loadConfig(*configPath)
	if err != nil {
		slog.Error("failed to load auth config", "config_path", *configPath, "error", err)
		os.Exit(1)
	}
	...
	if err != nil {
		logger.Error("init auth service failed", "error", err)
		os.Exit(1)
	}
	...
	go func() {
		if err := srv.StartGRPC(); err != nil {
			logger.Error("auth grpc server error", "error", err)
			os.Exit(1)
		}
	}()
